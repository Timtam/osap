...
- `modules/hello`, `modules/hotkey`, `modules/window`, `modules/window-trigger`, `modules/screen`, `modules/ocr`, `modules/input`, `modules/sound`, `modules/overlay`, `modules/overlay-attach`, `modules/keys`, `modules/settings`, `modules/sforzando` (the first real ReaHotkey port: a self-voicing OCR overlay over the standalone sforzando window, navigated with Tab), `modules/inspect` (OCR window inspector dev tool: Ctrl+Alt+I logs every recognized word with client-relative coordinates), `modules/daw-hosts` (a **library module**: shared DAW host-window matchers, imported by other modules via `host.require` — modules declare `dependencies` in `module.toml`, auto-loaded first) — example modules (speak-once; global-hotkey trigger; window/os inspection; foreground-change trigger; screen capture & image search; OCR; mouse/keyboard input; audio playback; self-voicing overlay; context-bound overlay; low-level key capture).
