...
menu-bar item and its menu has **Show module manager**); closing it hides it back to the tray, and the tray menu's **Quit** exits. Set `AUTOMATION_PLATFORM_HEADLESS=1` to run without any window (Ctrl+C to quit). Diagnostics are written to `automation-platform.log` next to the executable — next to the
