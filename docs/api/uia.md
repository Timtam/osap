---
title: "host.uia — the accessibility tree"
sidebar_position: 5
toc_max_heading_level: 2
---

The first question to ask about a window, and the only answer that survives it being moved, resized or redrawn: a published tree names its elements and gives their rectangles, where every coordinate a module writes down is a claim about one window size on one machine.
...
local function dumpOne(label, id)
  ...
  for _, e in ipairs(host.uia.dump(id)) do
...
local pt = fg and host.uia.locateVia(fg.id, "Kontakt 8", U.Pane, "Kontakt File Menu", U.Button)
if pt then
  host.input.click(pt.x, pt.y)
else
...
Control types are translated to accessibility roles, so an unmapped type at **either** level makes the call impossible — it can never match, and says so only in the log (see [`host.uia.type`](#host-uia-type)). Each level gets its own **1500 node / depth 24** budget, and an exhausted budget returns the same `nil` an honest miss returns.

## host.uia.stateProbe(hwnd, containerName, name, controlType) {#host-uia-stateprobe}

**Signature:** `host.uia.stateProbe(hwnd: number, containerName: string, name: string, controlType: number) -> { toggle: number?, legacyState: number, checked: boolean } | nil`
...
local fg = host.window.active()
local st = fg and host.uia.stateProbe(fg.id, "Kontakt 8", KEYBOARD, U.Button)
...
## host.uia.classNavPoint(hwnd, className, controlType, child, sibling) {#host-uia-classnavpoint}

**Signature:** `host.uia.classNavPoint(hwnd: number, className: string, controlType: number, child: number, sibling: number) -> { x: number, y: number } | nil`
...
local function browserToggle(hwnd)
  ...
  return host.uia.classNavPoint(hwnd, "FileTypeSelector", U.Tab, 0, -1)
    or host.uia.classNavPoint(hwnd, "TagCloudAccordionWithBrands", U.Pane, 0, -1)
end
```
...
## host.uia.focusStep(hwnd, direction) {#host-uia-focusstep}

**Signature:** `host.uia.focusStep(hwnd: number, direction: number) -> { name: string, ctype: number, index: number, count: number } | nil`
...
local r = ctrl and ctrl.id and host.uia.focusStep(ctrl.id, dir)
