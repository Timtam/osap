...
A help bubble that appears after a press is the same call with `before = nil`: anything found is new. [`host.screen.snapshotAsync`](screen.md#host-screen-snapshotasync) does the waiting in one call, off the event loop: with `change` it photographs the menu until the cursor column differs from the picture the module last read (`from`) or, without one, from the wait's own first picture, taken after the press was delivered; and with `at = e.time + 80` it takes a picture a set time after the press itself; its page has this menu written that way.
