...
**A key written twice in one object is not an error: the last one wins.** `{ "a": 1, "a": 3 }` gives `a == 3`, and a later `"a": null` removes an earlier value. A converter that writes a key twice by mistake is therefore not told.
...
**The options are strict.** `pretty` is the only one, and `false` or `nil` means compact. These raise before anything is written: an `opts` that is neither a table nor `nil` (`host.json.encode: options are a table, not a string`), any other key — a misspelling included (`host.json.encode: 'prety' is not an option; the only one is pretty`) — and a `pretty` that is not `true`, `false`, `nil` or a whole number from 1 to 8 (`0`, `9`, `2.5` and `"yes"` all raise: `pretty is true, false or an indent width from 1 to 8, not …`).
