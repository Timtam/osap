...
**Nachtrag Schritt 7 (2026-09-21): DXGI Desktop Duplication ist gebaut, aber nur als Opt-in pro Modul** (`[screen] capture = "duplication"` im `module.toml`, siehe `docs/api/screen.md#which-picture-a-read-sees`), nicht als Ersatz des BitBlt-Pfads. Anlass war nicht Tempo, sondern ein Spiel, dessen Bild ein GDI-Leser eingefroren sieht — ob auch *unser* GDI-Pfad es eingefroren liest, ist noch nicht gemessen (Schritt-0-Probe und M4 in `TODO.md`). Engine in `crates/host/src/backend/dxgi.rs` (eigener Thread `dxgi-capture`, Pull per `AcquireNextFrame(0)`, letzter Frame auf der GPU), ruhend, solange kein geladenes Modul sie verlangt; ein Schalter in den Application settings schaltet sie global ab. Erste Messung auf der Referenzmaschine (Test-Build, Release): kleine Regionen 0,3–1 ms statt 15–16 ms, Vollbild 5,4 ms statt 28 ms, Bytes identisch mit GDI. Offen sind die Feldmessungen M1–M19 in `TODO.md` (Spiel, Fullscreen-Wechsel, UAC, Screen Curtain, Hybrid-GPU, Mehrmonitor). Die Dirty-Rect-getriggerte Post-Toggle-Ansage (`Req::WaitChange`) ist reserviert, nicht gebaut.
