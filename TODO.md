...
- [ ] **Module-management window (next):** build the actual UI on the wxDragon window — list loaded modules with enable/disable toggles (the dispatcher already honors the per-module `enabled` flag), wired to the manager. This is the control surface for the item below.
- [ ] **Module manager follow-ups:** dynamic enable/disable/reload via the control surface (window above + CLI/IPC); persist the enabled set; conflict detection (duplicate hotkeys / same captured key in two modules). Out-of-process only for the untrusted-native-FFI tier. See [docs/module-runtime-and-lifecycle.md](docs/module-runtime-and-lifecycle.md).
