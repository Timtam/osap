...
- [ ] **Slice 5:** `host.window.onTrigger(matcher, {on=…}, cb)` — window event layer (Win `SetWinEventHook`) wired into the event loop → auto-activation (the foundation for overlays).
- [ ] **Slice 6:** `host.screen` (capture, image search, pixel) + `host.ocr` (Windows.Media.Ocr / ONNX) — the building blocks the ReaHotkey overlays need.
