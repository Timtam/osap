...
- [ ] **Slice 8:** `host.input` (mouse move/click/drag/scroll; key send/text) — Windows `SendInput`. Needed for overlay actions (Hotspot/Custom).
- [ ] **Slice 9:** `host.sound` (audio-asset playback) + `host.overlay` runtime (self-voicing virtual control tree) — completes the ReaHotkey overlay MVP.
