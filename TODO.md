...
- [ ] **Slice 11:** Context-scoped **key suppression** — low-level keyboard hook (`WH_KEYBOARD_LL`) so overlays can use Tab/arrows and intercept keys only while active (the natural ReaHotkey navigation, instead of the current non-conflicting Ctrl+Alt hotkeys).
